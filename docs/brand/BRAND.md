...
2. **No period on short statements.** `the fire is lit` — not `the fire is lit.` Periods only on multi-sentence copy.
3. **Personify the fire, not the bot.** Say `the fire is lit` or `bonfire's out`, not `I have started the server` or `the bot is online`. The bot is the keeper; the server is the fire.
...
| running     | lit · going · the fire is lit               | burning · UP · ONLINE · active      |
...
|-------------|----------------|----------------------------------------------------------------|
| `running`   | lit            | `lit for 1h 42m · by @otso`                                    |
...
## What ports to Discord, what doesn't

The brand book renders embeds in HTML with web fonts, custom colors, glows, and pills to show the *intended visual language*. Discord itself is more constrained. Know the split before implementing.

### Ports directly

- **Embed color strip** (left edge). Single RGB integer via `embed.color`. Use the palette hex codes as-is: `0xe8793a` for running, `0xf2c14e` for starting, `0x6b8f9c` for stopping, `0x9a8e7d` for stopped, `0xc65d4a` for errors.
- **Author block** — icon + name + optional URL. Use for `bonfire · keeper` branding with the avatar PNG.
- **Footer** — icon + text + timestamp. Good slot for `lit · just now`-style metadata.
- **Thumbnail / image** — standard image slots.
- **Fields** — name/value pairs. Set `inline: true` to lay ADDRESS / UPTIME / BACKUP in a row.
- **Markdown** — `**bold**`, `*italic*`, `` `inline code` ``, `> quote`, hyperlinks. Use italic for game names to approximate the Cormorant treatment.
- **Monospace for data** — wrap IPs / uptimes / timestamps in `` ` `` backticks. Renders in Discord's mono stack with a subtle dark box — acceptable tradeoff for the mono-data role.
- **One emoji** — 🔥 is sanctioned in the `running` state headline only. No others.
- **All copy, state vocabulary, voice rules, field structure, command descriptions** — these are the point of the brand book. They port verbatim.

### Does not port

- **Custom fonts.** Cormorant Garamond and Inter Tight won't render — Discord uses its own type stack (gg sans on web). The serif italic treatment in the book is aspirational.
- **Custom text colors inline.** You can't dim "ADDRESS" while brightening its value. Color lives only on the strip.
- **Custom backgrounds / gradients / glows.** The soft radial ember glow in the Hero mockup is HTML-only. Discord renders a flat card.
- **Custom pills / badges.** The `● lit` state pill is a fiction. Render as inline text: `Valheim — lit` or `Valheim · lit`.
- **Custom layout.** Discord controls placement. You get slots (title, description, fields, footer), not a canvas. The 2×2 field grid is a field layout hint, not a guarantee.

### Mental model

Think of the brand book as specifying **what the embed says, what its color is, and what information sits where** — not how the pixels arrange. A Discord embed that uses the right words, the right color strip, and the right field structure will *feel* right even rendered in gg sans on Discord's flat chrome.

When in doubt: the color strip + the copy are load-bearing. Everything else is bonus.
...
| *authorization failure*  | **Alert**     | ephemeral   | Gentle refusal, not punishment                    |
| *not_found*              | **Alert**     | ephemeral   | Same                                              |
| *idempotency race*       | **Line**      | ephemeral   | "it's already lit, here's the address"           |
| *AWS / Discord errors*   | **Alert**     | ephemeral   | Error class                                       |
...
- Read as what the bot *offers*, not what the command *does*
...
- `bonfire-unlit-*.png` — dormant state, logs + faint ember. Optional: swap via Discord's bot-avatar API as a state signal (if implementing: only when *all* fires in a guild are stopped).
...
This file mirrors the brand book version. Currently **v1.5**. Brand book is at the same version.

### Changelog

- **v1.5** — Added "What ports to Discord, what doesn't" section. Clarifies which brand elements survive the crossing to Discord's render (color strip, fields, markdown, copy) vs. which are aspirational (custom fonts, glows, pills, layout). Prevents implementing something Discord won't render.
- **v1.4** — `running` state label changed from `burning` → `lit`. "Burning" carried ops/incident baggage ("prod is burning"); `lit` is warmer, matches the verb system (`light the fire` → `lit`), and sits naturally with durations (`lit for 1h 42m`). `kindle` stays available as an expressive variant for *action* copy (not state).
